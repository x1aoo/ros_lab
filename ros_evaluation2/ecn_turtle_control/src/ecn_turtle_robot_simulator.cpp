...
void velocityCallback( geometry_msgs::Twist velocityCommand ){
    ...
    if( !velocityReceived ) {
        ...
        x     += velocityCommand.linear.x * deltaT.toSec() * cos(theta) ;
        y     += velocityCommand.linear.x * deltaT.toSec() * sin(theta) ;
        theta += velocityCommand.angular.z * deltaT.toSec()             ;
...
int main (int argc, char** argv)
{
    ...
    ros::Subscriber velSub    = nh_.subscribe<geometry_msgs::Twist>("/cmd_vel",1,velocityCallback) ;
    ros::Subscriber radiusSub = nh_.subscribe<std_msgs::Float32>("/radius",1,radiusCallback) ;
    ros::Publisher  errorPub  = nh_.advertise<geometry_msgs::Pose2D>("/pose_error",1) ;
                    posePub   = nh_.advertise<geometry_msgs::Pose2D>("/pose",1) ;

    x = x0 ;   y = y0 ;   theta = theta0 ;
    geometry_msgs::Pose2D poseError ;
